...
#        puts cf.values.first.size
        next unless user && group && !user.send(@criteria).nil?
...
class Classifier
  ...
  def results_to_db!
    @groups.each do |group|
      ...
      dm_group.timezone = "Lifehacker - #{group.timezone}"
      dm_group.save
    end
  end
  ...
  def results_to_file(file_name)
    open(file_name, 'w') do |file|
      ...
      @groups.each_with_index do |group, index|
        ...
        file.puts "\tTimezone: " + (group.timezone ? group.timezone : "no timezone")
        file.puts "\tUsers:"
        group.each do |user|
          file.puts "\t\tUser " + user.id.to_s + ": \t\t" + 
                    [user.email, (user.timezone ? user.timezone : "no timezone"), user.source, user.group_code].join(", ")
        end
      end
    end
  end
...
sms2.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == g.timezone.split('/').first) : false}
sms2.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Pacific' && g.timezone.split('/').first == 'Asia') : false}
sms2.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Africa' && g.timezone.split('/').first == 'Europe') : false}
sms2.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Etc' && g.timezone.split('/').first == 'Asia') : false}

sms2_no_tz = SpecificStrategy.new
sms2_no_tz.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == g.timezone.split('/').first) : true}
sms2_no_tz.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Pacific' && g.timezone.split('/').first == 'Asia') : true}
sms2_no_tz.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Africa' && g.timezone.split('/').first == 'Europe') : true}
sms2_no_tz.add_rule {|u, g| (u.timezone && g.timezone) ? (u.timezone.split('/').first == 'Etc' && g.timezone.split('/').first == 'Asia') : true}




fsgs = FoldSmallGroupStrategy.new
fsgs.hard_minimum = 25

all = SpecificStrategy.new
all.add_rule {|u, g| true }


c = Classifier.new(mgs, dis, sms, sms2, fsgs, dis, sms, sms2,
                   fsgs, dis, sms, sms2, sms2, sms2_no_tz,
                   fsgs, dis, sms, sms2, sms2, sms2_no_tz, all)
c.logger.level = Logger::INFO
c.users = User.all( :source => 'lifehacker', :group_id => nil) #, :order => [:id.desc], :limit => 5000)
c.classify!
c.results_to_file('groups-lifehackers.log')
